...
def box3D_denormalization(boxes, deltas, mean=None, std=None):
    ...
    boxes_exp = keras.backend.expand_dims(boxes, 2)

    width = boxes_exp[:, :, 0, 2] - boxes_exp[:, :, 0, 0]
    height = boxes_exp[:, :, 0, 3] - boxes_exp[:, :, 0, 1]

    x1 = boxes_exp[:, :, 0, 0] + (deltas[:, :, 0, 0] * std[0] + mean[0]) * width
    y1 = boxes_exp[:, :, 0, 1] + (deltas[:, :, 0, 1] * std[1] + mean[1]) * height
    x2 = boxes_exp[:, :, 0, 2] + (deltas[:, :, 0, 2] * std[2] + mean[2]) * width
    y2 = boxes_exp[:, :, 0, 3] + (deltas[:, :, 0, 3] * std[3] + mean[3]) * height
    x3 = boxes_exp[:, :, 0, 0] + (deltas[:, :, 0, 4] * std[4] + mean[4]) * width
    y3 = boxes_exp[:, :, 0, 1] + (deltas[:, :, 0, 5] * std[5] + mean[5]) * height
    x4 = boxes_exp[:, :, 0, 2] + (deltas[:, :, 0, 6] * std[6] + mean[6]) * width
    y4 = boxes_exp[:, :, 0, 3] + (deltas[:, :, 0, 7] * std[7] + mean[7]) * height
    x5 = boxes_exp[:, :, 0, 0] + (deltas[:, :, 0, 8] * std[8] + mean[8]) * width
    y5 = boxes_exp[:, :, 0, 1] + (deltas[:, :, 0, 9] * std[9] + mean[9]) * height
    x6 = boxes_exp[:, :, 0, 2] + (deltas[:, :, 0, 10] * std[10] + mean[10]) * width
    y6 = boxes_exp[:, :, 0, 3] + (deltas[:, :, 0, 11] * std[11] + mean[11]) * height
    x7 = boxes_exp[:, :, 0, 0] + (deltas[:, :, 0, 12] * std[12] + mean[12]) * width
    y7 = boxes_exp[:, :, 0, 1] + (deltas[:, :, 0, 13] * std[13] + mean[13]) * height
    x8 = boxes_exp[:, :, 0, 2] + (deltas[:, :, 0, 14] * std[14] + mean[14]) * width
    y8 = boxes_exp[:, :, 0, 3] + (deltas[:, :, 0, 15] * std[15] + mean[15]) * height

    pred_boxes = keras.backend.stack([x1, y1, x2, y2, x3, y3, x4, y4, x5, y5, x6, y6, x7, y7, x8, y8], axis=2)
    pred_boxes = keras.backend.expand_dims(pred_boxes, 2)
