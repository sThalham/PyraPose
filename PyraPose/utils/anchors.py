...
def anchor_targets_bbox(
    ...
    mask_locations = int((image_group[0].shape[0] * (1/8)) * (image_group[0].shape[1] * (1/8)))
    #print('mask locations: ', int(mask_locations))
                ...
                if np.sum(np.abs(sym_disc)) != 0:
                    for sdx in range(sym_disc.shape[0]):
                        if np.sum(np.abs(sym_disc[sdx, :])) != 0:
                            ...
                            calculated_boxes[sdx, :] = box3D_sym
...
def toPix_noncenter(translation, fx=None, fy=None, cx=None, cy=None):
...
def get_cont_sympose(rot_pose, sym):
    ...
    if sym[2] == 1:
        ...
        cam_in_obj = np.dot(np.linalg.inv(trans), (0, 0, 0, 1))
        alpha = math.atan2(cam_in_obj[2], cam_in_obj[0])
